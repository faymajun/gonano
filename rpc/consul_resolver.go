...
func (c *consulResolver) query(opts *consul.QueryOptions) ([]resolver.Address, uint64, error) {
	...
	if err != nil {
		log.Infof("grpcconsulresolver: resolving service name '%s' via consul failed: %v\n",
			c.service, err)
	...
	for _, e := range entries {
		...
		if addr == "" {
			addr = e.Node.Address
			log.Infof("grpcconsulresolver: service '%s' has no ServiceAddress, using agent address '%+v'", e.Service.ID, addr)
		}

		for _, v := range e.Service.Tags {
			strArr := strings.Split(v, "=")
			if len(strArr) == 2 && strArr[0] == "gRPC.port" {
				port := strArr[1]
	...
	log.Infof("grpcconsulresolver: service '%s' resolved to '%+v'", c.service, result)
	...
	for {
		for {
			...
			if err != nil {
				...
				// After ReportError() was called, the rpc
			...
			// set of resolved addresses. It informs the rpc-balancer that resolution is not
			// in progress anymore and rpc calls can failFast.
			if addressesEqual(addrs, lastReportedAddrs) {
				...
				if lastWaitIndex == opts.WaitIndex &&
					...
					log.Warningf("grpcconsulresolver: consul responded too fast with same data and waitIndex (%d) then in previous query, delaying next query",
						opts.WaitIndex)
