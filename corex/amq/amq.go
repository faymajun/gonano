...
func New(name string, host string, port int32, username string, password string,
	durable bool, autoDelete bool, exclusive bool, noWait bool) *Session {
	if username == "" && password == "" {
		username = "guest"
		password = "guest"
	}
	addr := fmt.Sprintf("amqp://%s:%s@%s:%d/", username, password, host, port)
	session := Session{
		name:       name,
		done:       make(chan bool),
		durable:    durable,
...
func (session *Session) handleReInit(conn *amqp.Connection) bool {
	for {
		...
		if err != nil {
			log.Info("Failed to initialize channel. Retrying...")
...
func (session *Session) init(conn *amqp.Connection) error {
	...
	_, err = ch.QueueDeclare(
		...
		session.autoDelete, // Delete when unused
		session.exclusive,  // Exclusive
		session.noWait,     // No-wait
		nil,                // Arguments
	)
...
func (session *Session) Push(data []byte, name string) error {
	if !session.isReady {
